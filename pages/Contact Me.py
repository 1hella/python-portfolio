...
with st.form(key="email_forms", clear_on_submit=True):
    ...
    name = st.text_input("Your name", key="name",
                         value=st.session_state['name'] if 'name' in st.session_state and not st.session_state['submitted'] else '')
    name_error_message = st.empty()
    email_address = st.text_input("Your email address", key="email",
                                  value=st.session_state['email'] if "email" in st.session_state and not st.session_state['submitted']
    else "")
    email_error_message = st.empty()
    message = st.text_area("Your message", key="message",
                           value=st.session_state['message'] if "message" in st.session_state and not st.session_state['submitted'] else "")
